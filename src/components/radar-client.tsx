...
import { ClasseFaixa, rotuloConfianca } from "./rotulos";
  ...
  useEffect(() => {
    ...
    void Promise.resolve(contexto.registerTool({
      ...
      async execute(entrada: unknown) {
        ...
        const permitidas = ["texto", "indiceMinimo", "municipio", "tipo", "variasUnidades", "expansao", "operacao24h", "qualidadeEvidencia", "faixa", "ordem"];
      ...
      <section aria-label="Filtros do radar" className="painel p-5">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          ...
          <div className="flex flex-wrap items-end gap-x-5 gap-y-3 xl:col-span-2">
            {[{ campo: "variasUnidades", rotulo: "Várias unidades" }, { campo: "expansao", rotulo: "Com expansão" }, { campo: "operacao24h", rotulo: "Opera 24h" }].map(({ campo, rotulo }) => <label className="flex min-h-11 cursor-pointer items-center gap-2 text-sm font-medium" key={campo}><input className="size-4 accent-[var(--azul)]" type="checkbox" checked={Boolean(filtros[campo as keyof FiltrosRadar])} onChange={(evento) => alterar(campo as keyof FiltrosRadar, evento.target.checked)} />{rotulo}</label>)}
          </div>
        </div>
