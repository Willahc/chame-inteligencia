  ...
  return (
    <div className="w-full px-3 py-5 sm:px-6 lg:px-8 lg:py-8 2xl:px-10">
      ...
      <section className="mt-7 grid gap-6 xl:grid-cols-[1.45fr_.75fr]">
        <article className="painel p-6">
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div>
              <p className="text-sm font-semibold text-[var(--texto-suave)]">Perfil da instituição</p>
              <h2 className="mt-1 text-xl font-bold text-[var(--azul-profundo)]">{instituicao.tipoEstabelecimento.nome}</h2>
              {instituicao.grupoEconomico ? (
                instituicao.grupoEconomico.tipoDado !== "DEMONSTRACAO" ? (
                  <p className="mt-2 text-sm">
                    <span className="text-[var(--texto-suave)]">Organização vinculada: </span>
                    <Link className="font-bold text-[var(--azul)] hover:underline" href={`/organizacoes/${instituicao.grupoEconomico.id}`}>
                      {instituicao.grupoEconomico.nome}
                    </Link>
                  </p>
                ) : (
                  <p className="mt-2 text-sm text-[var(--texto-suave)]">Organização: {instituicao.grupoEconomico.nome}</p>
                )
              ) : (
                <p className="mt-2 text-sm text-[var(--texto-suave)]">Instituição sem grupo econômico registrado</p>
              )}
            </div>
          ...
          <dl className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            <div className="rounded-xl bg-slate-50 p-4"><dt className="text-xs font-semibold uppercase tracking-wide text-[var(--texto-suave)]">Município</dt><dd className="mt-2 text-base font-bold">{municipios.join(", ") || "Não informado"}</dd></div>
            ...
            <div className="rounded-xl bg-slate-50 p-4"><dt className="text-xs font-semibold uppercase tracking-wide text-[var(--texto-suave)]">Cobertura</dt><dd className="mt-2 text-lg font-bold">{instituicao.coberturaDados}%</dd></div>
          ...
          <div className="mt-6"><h3 className="text-sm font-bold">Serviços registrados</h3><div className="mt-3 flex flex-wrap gap-2">{instituicao.servicos.map((servico) => <span className="rounded-lg bg-[var(--ciano-claro)] px-3 py-1.5 text-sm font-medium text-[var(--azul)]" key={servico.id}>{servico.nome}</span>)}</div></div>
        </article>
      ...
      <section className="painel mt-6 p-6">
        <h2 className="text-lg font-bold">Contatos comerciais</h2>
